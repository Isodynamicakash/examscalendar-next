...
export default function QuestionSolver({ q, answer, examLabel, chapterName, chapterHref, basePath, filterQuery = "", slugList = [] }) {
  ...
  // Bookmark state: null = unknown/loading, true/false = confirmed.
  // "user" is either the logged-in Supabase user or null (logged out).
  ...
  const resetState = useCallback(() => {
    ...
    setAnswerShown(false); setSolutionShown(false); setFlash(null); setSeconds(0);
  ...
  // Load auth state + whether THIS question is already bookmarked.
  // Runs on every question change so Next/Previous shows the right state.
  useEffect(() => {
    let cancelled = false;
    (async () => {
  ...
  const toggleBookmark = async () => {
    ...
    try {
      if (bookmarked) {
        ...
        // user_id is set automatically by RLS INSERT check + a sensible
        // default -- but we set it explicitly here since the policy
        // requires WITH CHECK (auth.uid() = user_id).
  ...
  const isCorrect = () => {
    if (isNumerical) { const g = parseFloat(numericInput), e = parseFloat(answer?.correct_option); return !isNaN(g) && !isNaN(e) && Math.abs(g - e) < 0.01; }
    if (isMSQ) { const a = [...selectedMulti].sort(), b = [...correctOptions].sort(); return a.length === b.length && a.every((v, i) => v === b[i]); }
  ...
  const showSolution = () => { clearInterval(timerRef.current); setSolutionShown(true); };
  const toggleMulti = (i) => { if (answerShown) return; setSelectedMulti((prev) => (prev.includes(i) ? prev.filter((x) => x !== i) : [...prev, i])); };
  ...
  const optState = (i) => { if (!answerShown) return isSelectedOpt(i) ? "sel" : "def"; if (isCorrectOpt(i)) return "ok"; if (isSelectedOpt(i)) return "bad"; return "dim"; };
  const OPT = {
  ...
  // Bookmark button appearance: disabled-look when logged out, filled
  // gold when saved, hollow when not.
      ...
      <div style={{ background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: 14, padding: "20px 22px", ...flashOverlayStyle }}>
        ...
        {!isNumerical && opts.length > 0 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 8 }}>
            {opts.map((opt, i) => { const st = OPT[optState(i)]; return (
              <button key={i} onClick={() => (isMSQ ? toggleMulti(i) : !answerShown && setSelected(i))} disabled={answerShown} style={{ display: "flex", alignItems: "flex-start", gap: 12, textAlign: "left", padding: "12px 16px", borderRadius: 10, border: `1.5px solid ${st.border}`, background: st.bg, color: st.color, cursor: answerShown ? "default" : "pointer", fontSize: 15 }}>
        ...
        {isNumerical && (
          <div style={{ marginTop: 8 }}>
            ...
            <input type="number" step="any" value={numericInput} onChange={(e) => !answerShown && setNumericInput(e.target.value)} disabled={answerShown} placeholder="e.g. 12.5" style={{ width: "100%", padding: "12px 14px", borderRadius: 10, fontSize: 16, border: `1.5px solid ${answerShown ? (isCorrect() ? T.green : T.red) : T.border}`, background: T.surface, color: T.text, outline: "none" }} />
            {answerShown && <div style={{ marginTop: 8, fontSize: 13, color: T.textMuted }}>Correct answer: <strong style={{ color: T.greenText }}>{answer?.correct_option}</strong></div>}
          </div>
        )}

        <div style={{ display: "flex", gap: 10, marginTop: 18, flexWrap: "wrap" }}>
          {!answerShown && <button onClick={showAnswer} disabled={!hasAnswer} style={{ flex: 1, minWidth: 140, padding: "13px", borderRadius: 10, fontSize: 14, fontWeight: 700, border: "none", cursor: hasAnswer ? "pointer" : "not-allowed", background: hasAnswer ? T.accent : T.surfaceHigh, color: hasAnswer ? "#fff" : T.textDim }}>Show Answer</button>}
          {!solutionShown && <button onClick={showSolution} style={{ flex: 1, minWidth: 140, padding: "13px", borderRadius: 10, fontSize: 14, fontWeight: 700, border: `1px solid ${T.purple}`, cursor: "pointer", background: "transparent", color: T.purple }}>Show Solution</button>}
          {(answerShown || solutionShown) && <button onClick={resetState} style={{ flex: 1, minWidth: 140, padding: "13px", borderRadius: 10, fontSize: 14, fontWeight: 700, border: `1px solid ${T.border}`, cursor: "pointer", background: "transparent", color: T.textMuted }}>↺ Solve Again</button>}
        </div>
